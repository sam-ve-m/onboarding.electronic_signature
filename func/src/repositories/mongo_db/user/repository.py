...
class UserRepository(MongoDbBaseRepository):
    ...
    @classmethod
    async def update_one_with_electronic_signature(cls, unique_id: str, user_electronic_signature):
        collection = await cls._get_collection()
        try:
            user_updated = await collection.update_one({"unique_id": unique_id}, {"$set": user_electronic_signature})
            return user_updated
        except Exception as ex:
            message = f'UserRepository::update_one_with_user_complementary_data::error on update complementary data":' \
